...
function defaultDelayStrategy(n) { return 1000; }
...
@param {Object} settings For detailed information about each option check documentation.
  ...
  @param {Number} [settings.max_upload_slots=1] How many concurrent uploads to queue.
	@param {Function} [settings.delayStrategy=defaultDelayStrategy] Function returning ms to wait for retrying for failed upload.
		...
		plupload.extend(this, {
			...
			*/
			upload: function(options) {
				...
				, offset = 0
				, delayStrategy = (typeof options.delayStrategy === 'function') ?
						options.delayStrategy : defaultDelayStrategy
				;

				file.status = plupload.UPLOADING;

				function handleError(_xhr) {
					if (retries-- > 0) {
						let delayMs = -1;
						const retryAfterSlots = (
							_xhr.getResponseHeader('Retry-After') || '').split(':');
						// implement Retry-After rfc
		        // http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.37
		        if (retryAfterSlots.length === 2) {
		          // it must be a date
		          if (isNaN(retryAfterSlots[1])) {
		            delayMs = new Date(retryAfterSlots[1]).getTime() - $.now();
		          // its a number in seconds
		          } else {
		            delayMs = parseInt(retryAfterSlots[1], 10) * 1000;
		          }
		        }

	          if (isNaN(delayMs) || delayMs < 0) {
	          	delayMs = defaultDelayStrategy(options.max_retries - retries);
	          }

						delay(uploadNextChunk, delayMs);
					} else {
				...
				function uploadNextChunk() {
					...
					xhr.onload = function(event) {
						...
						retries = options.max_retries; // reset the counter
